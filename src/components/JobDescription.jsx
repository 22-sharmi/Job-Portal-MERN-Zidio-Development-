        ...
        const fetchJobAndCompany = async () => {
            try {
                ...
                if (res.data.success) {
                    ...
                    const companyRes = await axios.get(`${COMPANY_API_END_POINT}/get/${res.data.job.company}`, { withCredentials: true });
                    if (companyRes.data.success) {
                        setCompanyName(companyRes.data.company.name); // Set the company name
                    }
                }
            } catch (error) {
                console.log(error);
        ...
        <div className='max-w-7xl mx-auto my-6 px-4 sm:px-6 lg:px-8'>
            <div className='flex flex-col sm:flex-row items-start justify-between gap-4'>
                ...
                <div className='flex flex-col items-start'>
                    ...
                    <p className='text-sm text-gray-600'>{companyName || 'Company name not available'}</p> {/* Updated */}
